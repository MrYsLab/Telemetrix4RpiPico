...
#define USQRTBOOST 1  // a hack: set to 1 for faster convergence when 1<iterN<5
...
inline uint32_t udiff(uint32_t a, uint32_t b) {
    ...
    while (true) {
		...
		if (timer10khz_triggered) {
			...
			for (uint8_t m=0; m<MAX_STEPPER_COUNT; m++) {
				if (stepper[m].dir_pin) {  // one running stepper takes ca. 450 CPU cycles (if not messaging)
					...
					for (uint8_t j=0; j< abs((new_nanopos/NANOSTEP_PER_MICROSTEP) -
						...
						// no-wait stepstick driving seems safe (at 125MHz)
						gpio_put(stepper[m].step_pin, 1); // 50ns pulse seems OK (or busy_wait_us_32(1);  )
